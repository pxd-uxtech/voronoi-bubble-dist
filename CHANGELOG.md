...
- **v2.3.0에서 group 셀 외곽선 도형이 어긋나던 문제**: 교차점의 stub 변을 합칠 때 기준을 "폴리곤 중앙값 변 길이의 35%"로 잡은 탓에, 점이 적은 group 셀에서 임계값이 커져 **실제 변까지 삼켰다**(예: 점 8개·중앙값 124px 셀에서 임계 43.6px이 24px짜리 실제 변을 제거 → 외곽선이 셀에서 최대 9.2px 벗어남). 이제 stub 판정을 **양옆 변 대비 15% 미만**으로 바꿔, 긴 변 사이에 낀 토막만 합친다. 합칠 때는 두 점의 중점을 쓰므로 폴리곤 순회 방향에 따라 모서리 위치가 달라지지 않는다.
